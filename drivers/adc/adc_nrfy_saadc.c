...
struct driver_data {
	...
	/* Temporary solution - channels configured by user */
	nrfx_saadc_channel_t channel_cfg[SAADC_CH_NUM]; // temporary solution
...
/* Temporary solution - configured channels for active measurement */
static nrfx_saadc_channel_t m_nrfx_config_data[SAADC_CH_NUM];
 ...
 */
static uint8_t adc_nrfx_config_structure_set(uint32_t channel_mask)
{
	uint8_t cnt = 0;
	...
	for (uint8_t i = 0; i < SAADC_CH_NUM; i++) {
		...
		if (m_data.configured[i]) {
			m_nrfx_config_data[cnt++] = m_data.channel_cfg[i];
		}
	}
				  ...
				  const struct adc_channel_cfg *channel_cfg)
{
	...
	cfg = &m_data.channel_cfg[channel_id];
		      ...
		      const struct adc_sequence *sequence)
{
	...
	channels_cnt = adc_nrfx_config_structure_set(selected_channels);
	if (channels_cnt == 0) {
		LOG_ERR("No channel configured");
		return -EINVAL;
	}

	nrfx_err = nrfx_saadc_channels_config(m_nrfx_config_data, channels_cnt);
	if (nrfx_err != NRFX_SUCCESS) {
		LOG_ERR("Channels configuration mismatch: %d", nrfx_err);
	}
