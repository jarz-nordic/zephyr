...
static int compare_long_option(const char *cmdarg,
			       const char *optname,
			       const char **argument)
{
	int result;
	...
	*argument = NULL;
	...
	for (; ; ) {
		...
		if (result != 0 || cmdchar == '\0') {
			/* If the '=' is the real terminator, then
			 * return the argument that follows the '='
			 */
			if (rawchar == '=') {
				*argument = cmdarg;
				}
 ...
 *
 */
static int getopt_long_option(struct getopt_s *go,
			      char * const argv[],
			      ...
			      int *longindex)
{
	...
	for (ndx = 0; longopts[ndx].name != NULL; ndx++) {
		...
		/* Skip over the option + argument */
			...
			/* Does the option have a required argument in the
			 * next argv entry? An optional argument?
			 */
			switch (longopts[ndx].has_arg) {
			char *next;
			case no_argument:
				/* No, no arguments. Just return the argument
				 * that we found.
				 */
				go->go_optptr = NULL;
				go->go_optind++;
				break;
			case optional_argument:
				/* Check if there is a following argument and
				 * if that following argument is another option.
				 */
		  ...
		  enum getopt_mode_e mode)
{
	...
	if (argv != NULL) {
		...
		while (!go->go_optptr || !*go->go_optptr) {
			/* We need to start at the beginning of the next argv.
			 * Check if we need to increment optind
			 */
			if (go->go_optptr) {
		...
		if (GETOPT_HAVE_LONG(mode)) {
			...
			if (*go->go_optptr == '-') {
				...
				if (ret != '?') {
					 ...
					 *
					 * REVISIT:  There is no way to distinguish a sequence
					 * of short arguments like -abc (meaning -a -b -c)
					 * from a single long argument (like "abc"). I am not
					 * sure of the correct behavior in this case.
					 * While supported for getopt(), I do not think that
					 * the first interpretation is standard.
					 */
